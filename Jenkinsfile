pipeline {
   ...
   stage('Wait for Master and Worker to Warm Up') {
	...
	stage('Generate variables for master and worker ip') {
		...
		sh 'scp -i linkedtoworld/linkedtoworld.pem -o StrictHostKeyChecking=no linkedtoworld/linkedtoworld.pem ec2-user@${MASTERIP}:/home/ec2-user/' 
		sh 'ssh -i linkedtoworld/linkedtoworld.pem -o StrictHostKeyChecking=no ec2-user@${MASTERIP} "scp -i ~/linkedtoworld.pem -o StrictHostKeyChecking=no /tmp/kubeadmjoin.sh ec2-user@${env.WORKERIP}:/home/ec2-user/"'
