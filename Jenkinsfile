pipeline {
   ...
   stage('Wait for Master and Worker to Warm Up') {
	...
	WORKERIP = ""
		...
		sh 'scp -i linkedtoworld.pem -o StrictHostKeyChecking=no ec2-user@${MASTERIP}:/home/ec2-user/' 
		sh 'ssh -i linkedtoworld.pem -o StrictHostKeyChecking=no ec2-user@${MASTERIP} "scp -i ~/linkedtoworld.pem -o StrictHostKeyChecking=no /tmp/kubeadmjoin.sh ec2-user@${env.WORKERIP}:/home/ec2-user/"'
